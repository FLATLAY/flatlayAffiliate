...
swaggerTools.initializeMiddleware(swaggerDoc, function (middleware) {
  ...
  // upload given file in floder
    ...
    if(typeof req.body.fileURL !== 'undefined'){
      ...
      imageFile.mv(`${filePath}/${fileUniqueName}`, function(err) {
			if (err) {
	...
	//https://affiliate.flat-lay.com/public/profile-images/
     var profileImage = fileUniqueName;
	...
	//http://affiliate.flat-lay.com/public/hero-images/
	app.post('/heroimage', (req, res, next) => {
		var fileUniqueName = 	uuidv1();
		var campaignID = req.body.campaignID;
		var name = req.body.name;
		var userID = req.body.userID;
		let response = {};
		let uploadPath = `public/hero-images`;;
		...
		let imageFile = req.files.file;
      	imageFile.mv(`${uploadPath}/${fileUniqueName}`, function(err) {
			if (err) {
				res.status(400).send(err);
			}else{
				connection.query('INSERT INTO tbl_campaigns_hero_images (image,campaignID,uid,name) VALUES(?,?,?,?)',
				[fileUniqueName, campaignID, userID,name],
				function (err, result) {
					if (!err) {
						res.json({ result: 'success' });
					} else {
  ...
  app.post('/webhook/shop/redact', (req, res) => {
	...
	app.get('/shopify', (req, res) => {
      ...
      const installUrl = 'https://' + shop +
				'.myshopify.com/admin/oauth/authorize?client_id=' + APIKEY +
				'&scope=' + scopes +
				'&state=' + state +
